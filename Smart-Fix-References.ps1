...
Get-ChildItem -Path $RootPath -Recurse -Filter "*.csproj" | ForEach-Object {
    ...
    Write-Verbose "Found project: $projectName at $($_.Directory.Name)"
}
...
function Find-BestMatch($referencePath, $fromPath) {
    ...
    Write-Verbose "Looking for best match for: $refProjectName"
    ...
    foreach ($name in $namesToTry) {
        if ($projectsByName.ContainsKey($name)) {
            ...
            if ($candidates.Count -eq 1) {
                Write-Verbose "Found exact match by name: $name"
                return $candidates[0]
            }
            ...
            foreach ($candidate in $candidates) {
                ...
                if ($candidateDepth -eq $originalDepth) {
                    Write-Verbose "Found match with same path depth: $candidate"
                    return $candidate
                }
            }
            
            # If no perfect match, return the first one
            Write-Verbose "Using first available match: $($candidates[0])"
            return $candidates[0]
        }
    }
