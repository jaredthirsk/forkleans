...
namespace Forkleans.Configuration
{
    ...
    public class AdoNetClusteringClientOptionsValidator : IConfigurationValidator
    {
        ...
        public void ValidateConfiguration()
        {
            ...
            {
                throw new ForkleansConfigurationException($"Invalid {nameof(AdoNetClusteringClientOptions)} values for {nameof(AdoNetClusteringTable)}. {nameof(options.Invariant)} is required.");
            }

            if (string.IsNullOrWhiteSpace(this.options.ConnectionString))
            {
                throw new ForkleansConfigurationException($"Invalid {nameof(AdoNetClusteringClientOptions)} values for {nameof(AdoNetClusteringTable)}. {nameof(options.ConnectionString)} is required.");
            }
        }
    }
}