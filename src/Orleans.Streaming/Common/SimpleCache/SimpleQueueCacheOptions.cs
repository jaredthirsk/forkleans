    ...
    public class SimpleQueueCacheOptionsValidator : IConfigurationValidator
    {
        ...
        public void ValidateConfiguration()
        {
            if(options.CacheSize <= 0)
                throw new ForkleansConfigurationException($"{nameof(SimpleQueueCacheOptions)} on stream provider {this.name} is invalid. {nameof(SimpleQueueCacheOptions.CacheSize)} must be larger than zero");
        }
